...
class SdbPost < AWS::Record::Model
  ...
  def self.fetch_by_tag(tag,start_time)

    sdb = AWS::SimpleDB.new

    posts = sdb.domains["posts_10"].items.select(:all).where("created>? and positive_tags = ?",start_time,tag)
    save_posts_features_and_tags(posts,tag,1);    

    posts = sdb.domains["posts_10"].items.select(:all).where("created>? and negative_tags = ?",start_time,tag)
    save_posts_features_and_tags(posts,tag,0);    
