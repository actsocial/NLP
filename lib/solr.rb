...
module Solr
	class Solr
		...
		def self.count_for_keywords_match(brand,keywords,date_start,date_end,min_match)
      results = []
      uri = "http://176.32.90.31:8983/solr/collection1/select?q=\"#{keywords.join('" "')}\"&fq=body_text:\"#{brand}\" AND date_dts:[\"#{date_start.strftime("%FT%R:%SZ")}\" TO \"#{date_end.strftime("%FT%R:%SZ")}\"]&rows=0&wt=json&indent=true&defType=dismax&qf=body_text&mm=#{min_match}"
      if(brand.index('and') || brand.index('or') || brand.index('AND') || brand.index('OR'))
        uri = "http://176.32.90.31:8983/solr/collection1/select?q=\"#{keywords.join('" "')}\"&fq=body_text:#{brand} AND date_dts:[\"#{date_start.strftime("%FT%R:%SZ")}\" TO \"#{date_end.strftime("%FT%R:%SZ")}\"]&rows=0&wt=json&indent=true&defType=dismax&qf=body_text&mm=#{min_match}"
      end
